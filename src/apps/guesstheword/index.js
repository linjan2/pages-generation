function getwords(type) {
  ...
  let url = `https://raw.githubusercontent.com/linjan2/data/main/${type}/${number}.json`;
...
function fetchDefinition(word) {
  ...
  return fetch(url, {method: "GET",headers: {"Accept": "application/json"}}).then(response => {
    if (response.ok) {
      return response.json();
    } else {
      console.error(response);
...
PetiteVue.createApp({
  ...
  reload() {
    ...
    this.input = "";
    this.solved = false;
    // delay setting definition=null to first disable rendering with v-if
  ...
  add(text) {
    ...
    if (!this.solved && text.length===word.length) {
      text = text.toLowerCase().padEnd(word.length,' ').substring(0,word.length);
      let input = [...text];
      let letters = [];
      ...
      if (text === this.answer) {
        ...
        if (this.currentType === "words") {
          fetchDefinition(text).then(definition => this.definition = definition);
        }
      }
      this.input = "";
    }
