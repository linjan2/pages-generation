...
function recalculate(items) {
  ...
  // use half-rotation to align sector middle to axis (and to allow odd number of sectors)
  let wyhalf = -Math.tan(Math.PI/(N*2)); // negative y since SVG y increases downwards (still rotates counter-clockwise)
  ...
  return {z: {x: zx, y: zy}, zhalf: {x: zxhalf, y: zyhalf }, wyhalf: wyhalf, rotations: rotations };
}
...
PetiteVue.createApp({
  ...
  add(text) {
    if (text !== "") {
      ...
      ({z: this.Z, zhalf: this.zhalf, wyhalf: this.wyhalf, rotations: this.R } = recalculate(this.items));
    }
  },
  remove(i) {
  ...
  playButton() {
    if (!this.playing) {
      ...
      let yo = (Math.random() * 2 - 1) * (this.wyhalf*0.95); // (-wyhalf, +wyhalf)
